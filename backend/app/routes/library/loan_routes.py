...
def create_loan():
    ...
    if item['status'] != ItemStatus.AVAILABLE.value:
        raise ValidationError('Item không khả dụng')
...
def renew_loan(loan_id):
    ...
    if loan['status'] != LoanStatus.ONGOING.value:
        raise ValidationError('Không thể gia hạn loan này')

    # Check if overdue
    if datetime.now(timezone.utc) > loan['due_date']:
        raise ValidationError('Không thể gia hạn sách quá hạn')
